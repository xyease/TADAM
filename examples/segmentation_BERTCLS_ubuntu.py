...
def write_segmentation_result(documents,dataset):
    # print(all_cut_list)
    with open(args.datapath+"14cutlist_"+dataset+".json",'r') as rf:
        all_cut_list=json.loads(rf.read())
    with open(args.datapath+"14cutresult_"+dataset+".txt",'w') as wf:
...
def document_segmentation():
    for dataset in ["train","test"]: #["test1"]:["train","valid","test"]
        print("start",dataset)
        documents=[]
        ...
        with open(args.datapath+"14cutlist_"+dataset+".json",'w') as wf:
            wf.write(json.dumps(all_cut_list, ensure_ascii=False))
